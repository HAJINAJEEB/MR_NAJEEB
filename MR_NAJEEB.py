...
try:
	key1 = open('/data/data/com.termux/files/usr/bin/.MR_NAJEEBahsan-cov', 'r').read()
except:
	kok=open('/data/data/com.termux/files/usr/bin/.MR_NAJEEBahsan-cov', 'w')
	kok.write(myid+imt)
	kok.close()
def login():
	try:
...
def first():
	clear()
	print(logo);print( ' [!] \033[1;96mON/OFF MODE PESAWAT JIKA TIDAK ADA MR_NAJEEB\033[1;0m\n')
...
def name2():
	clear()
	print(logo);print( '\n [O] MR_NAJEEB OK TERSIMPAN DI : \033[1;92mOK/%s\033[1;97m\n [C] MR_NAJEEB CP TERSIMPAN DI : \033[1;91mCP/%s\033[1;97m\n [O] \033[1;96mON/OFF MODE PESAWAT JIKA TIDAK ADA MR_NAJEEB\033[1;0m\n'%(okc,cpc))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[ MR_NAJEEB ] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
